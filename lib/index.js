...
class YandexGPTWriter {
  constructor() {
    ...
    this.modelUri = `llm://${this.catalogId}/yandex-lite`;
    this.iamToken = null;
  }
      ...
      const req = https.request(this.iamTokenApiUrl, options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            reject(new Error(`Error parsing IAM token response: ${error.message}`));
          }
        });
      });
  ...
  async writeYandex() {
    if (!this.apiUrl) {
      throw new Error("API URL is not defined.  Make sure REACT__API_URL is set in your environment.");
    }

    if (!this.catalogId) {
      throw new Error("Catalog ID is not defined.  Make sure REACT_YANDEX_CATALOG_ID is set in your environment.");
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = https.request(this.apiUrl, options, (res) => {
        ...
        res.on('end', () => {
          try {
            const parsedData = JSON.parse(data);
            const answerMessage = parsedData?.result?.alternatives?.[0]?.message;
            const answerText = answerMessage?.text;
